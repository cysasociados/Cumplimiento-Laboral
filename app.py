...
# --- CABECERA CON LOGOS ---
# Usamos columnas para poner los logos arriba a los extremos
...
anio_global = st.sidebar.selectbox("Año de Análisis", ["2026", "2025"])

# Cargamos el avance temprano para obtener los meses para el sidebar
df_av = cargar_datos(ID_AVANCE, anio_global)
cols_m_sidebar = [c for c in df_av.columns if c in MAPA_MESES.keys()] if not df_av.empty else []

# FILTRO DE MES EN SIDEBAR (DEBAJO DEL AÑO)
mes_sidebar = st.sidebar.selectbox("Mes de Análisis", ["AÑO COMPLETO"] + cols_m_sidebar)
...
tabs = st.tabs(["📈 Avance Laboral", "🏢 KPIs Empresas", "👥 Masa Colaboradores", "⚙️ Administración"]) if rol == "ADMIN" else st.tabs(["📈 Avance Laboral", "👥 Masa Laboral"])
...
with tabs[0]:
    ...
    if not df_av.empty:
        ...
        # Filtro de periodo basado en Sidebar
        ...
        st.header(f"Gestión de Control Laboral CMSG") # TEXTO CORREGIDO
        ...
        st.write("### 📊 Resumen de Estados por Periodo") # TEXTO CORREGIDO
...
if rol == "ADMIN":
    with tabs[3]:
        st.subheader("⚙️ Usuarios del Sistema")
