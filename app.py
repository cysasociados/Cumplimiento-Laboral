...
# URL DEL APPS SCRIPT (Verificada)
...
# --- FUNCIONES DE CARGA Y NORMALIZACIÓN TOTAL ---
@st.cache_data(ttl=10) # Cache corto para que tome tus cambios del Excel rápido
def cargar_datos(sheet_id, nombre_pestana):
    try:
        url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet={nombre_pestana}"
        df = pd.read_csv(url)
        # LIMPIEZA EXTREMA DE COLUMNAS: Sin espacios, todo a MAYÚSCULAS
...
if rol == "ADMIN":
    tabs = st.tabs(["📈 Avance Laboral", "🏢 KPIs Empresa", "👥 Masa Laboral", "⚙️ Log de Transacciones"])
elif rol == "REVISOR":
    tabs = st.tabs(["📈 Avance Laboral", "🏢 KPIs Empresa", "👥 Masa Laboral"])
else:
    tabs = st.tabs(["📈 Mi Avance", "👥 Masa Laboral"])

# --- TAB 1: AVANCE LABORAL (CON GRÁFICOS E INDICADORES) ---
with tabs[0]:
    if not df_av.empty:
        # Meses estándar para buscar en columnas
        ...
        k3.metric("% Avance Real", f"{(cumple_v/total_eval*100 if total_eval > 0 else 0):.1f}%")
        ...
        with col_a:
            ...
            # Buscamos la columna OBSERVACIONES que agregaste
            ...
            if mes_sel == "AÑO COMPLETO": st.info("Elija un mes en la barra lateral.")
                ...
                if not match_id.empty and 'ID_CARPETA' in df_id.columns:
                    ...
                    if id_f and len(id_f) > 10:
                        # Corregido: Obtenemos el número del mes del selector
                        num_m = str(meses_std.index(mes_sel) + 1).zfill(2)
                        nombre_pdf = f"Certificado.{num_m}{anio}"
                        if st.button(f"🔍 Descargar PDF {mes_sel}"):
                            with st.spinner("Buscando..."):
                                ...
                                if r.text.startswith("http"):
                                    ...
                                    st.session_state["log_accesos"].append({"FECHA": datetime.now().strftime("%d/%m/%Y %H:%M"), "USUARIO": st.session_state["u_nom"], "ACCION": f"Descarga {emp_sel} {mes_sel}"})
                                else: st.warning("Certificado no disponible en Drive.")
        ...
        for m in (cols_m if mes_sel == "AÑO COMPLETO" else [mes_sel]):
            ...
            if v in mapa_n: pie_data.append({'Estado': mapa_n[v]})
            else: pie_data.append({'Estado': "S/I"})
...
# --- TAB 2: KPIs EMPRESA (BASE IDS) ---
if rol != "USUARIO":
    with tabs[1]:
        st.header("🏢 Datos de Conexión de Empresas")
        st.dataframe(df_id, use_container_width=True)

# --- TAB 3: MASA LABORAL ---
with (tabs[2] if rol != "USUARIO" else tabs[1]):
    st.header("👥 Gestión de Masa Laboral")
    st.info("Módulo para análisis de dotación de trabajadores.")

# --- TAB 4: LOG DE TRANSACCIONES ---
if rol == "ADMIN":
    with tabs[3]:
        st.header("⚙️ Registro de Accesos y Descargas")
