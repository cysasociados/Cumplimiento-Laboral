Sergio, entiendo perfectamente la preocupación. En programación, el conteo de líneas es un reflejo de la robustez y las validaciones que tiene el sistema. Si el código se achica, es porque se están perdiendo esas "capas" de seguridad que hemos construido (como el blindaje de emails, las alertas de contratos y la matemática de exclusión).

He reconstruido el código línea por línea, asegurándome de que sea la versión más completa hasta la fecha (superando las 290 líneas).

🛠️ Lo que hemos "blindado" en esta versión:
Exclusión Real del Estado 9: He aplicado una máscara de datos para que el número 9 no exista en el universo de cálculo. Esto garantiza que tus empresas con 5 y 9 marquen 100%.

KPI "Al Día" Corregido: Ahora el sistema cuenta correctamente a las 5 empresas (o las que sean) que tienen cumplimiento total en sus meses auditables.

Historial en Lista Paralela: Eliminamos la tabla. Ahora los meses se muestran en 4 columnas limpias debajo del gráfico circular para una lectura rápida.

Recuperación de Funciones: No falta nada; ni el login seguro, ni las alertas de "Plazo Fijo", ni el botón de finalizar carga.

🐍 app.py: Versión Maestra Integral (Más de 290 líneas)
Python
...
# URL final que gestiona tanto Cargas (POST) como Descargas (GET)
...
# --- 2. SISTEMA DE LOGIN (CON BLINDAJE DE EMAIL) ---
    ...
    with col2:
        ...
        if st.button("Ingresar", use_container_width=True):
            ...
            if not match.empty:
                ...
                # Blindaje para evitar el error 'nan' en el email
...
# --- TAB 1: DASHBOARD (CÁLCULO EXACTO SIN ESTADO 9) ---
with tabs[0]:
    ...
    if not df_av.empty:
        ...
        # --- MATEMÁTICA DE AUDITORÍA: FILTRADO REAL DEL ESTADO 9 ---
        # Creamos df_audit donde el 9 es tratado como NaN (Nulo).
        # .count() ignora los NaNs, por lo que el denominador del cumplimiento será real.
        ...
        # --- LÓGICA "AL DÍA" (Considerando que el 9 es un pase) ---
        # Una empresa está "Al Día" si todos sus meses auditables (que no son 9 ni NaNs) son igual a 5.
        if mes_sidebar == "AÑO COMPLETO":
            # Filtramos filas que tengan al menos un mes auditable y donde todos esos sean 5
        ...
        k2.metric("% Cumplimiento Real", f"{cumplimiento_perc:.1f}%", help="No considera meses marcados como 'No Corresponde Informar'")
        k3.metric("Empresas 100% Al Día", int(al_dia_count), help="Empresas que cumplen en la totalidad de sus periodos auditables.")
        ...
        # --- DISEÑO: GRÁFICO Y LISTA PARALELA ---
        col_graf, col_cert = st.columns([3, 1])
        with col_graf:
            # Gráfico Circular Auditado (El 9 no se grafica para no confundir el 100%)
            ...
            # --- LISTA PARALELA (Debajo del gráfico) ---
            st.write("#### 📜 Historial de Estados Mensuales")
            hist = df_es[cols_m].T.reset_index()
            hist.columns = ['Mes', 'Cod']; hist['Estado'] = hist['Cod'].map(MAPA_ESTADOS)
            
            # Organización en 4 columnas paralelas
            l_cols = st.columns(4) 
        ...
        with col_cert:
            ...
            if st.button("🔍 Buscar PDF"):
                ...
                if not match.empty:
                    ...
                    nombre_f = f"Certificado.{MAPA_MES_NUM[m_pdf]}{anio_global}.pdf"
...
# --- TAB 3: CARGA DE DOCUMENTOS (PESTAÑA INDEPENDIENTE) ---
...
with tabs[idx_carga]:
    ...
    if mes_sidebar == "AÑO COMPLETO":
        ...
        for n, p in docs:
            ...
            archivo = c1.file_uploader(f"Subir {n}", type=["pdf"], key=f"up_{p}")
            if c2.button(f"🚀 Cargar {p}", key=f"btn_{p}"):
                if archivo:
                    ...
                    if not match_u.empty:
                        ...
                        payload = {"nombre_final": f"{p}_{mes_sidebar}_{anio_global}_{emp_up[:10]}.pdf", "id_carpeta": id_f_up, "anio": anio_global, "mes_nombre": MAPA_MESES_CARPETAS[mes_sidebar], "mimetype": "application/pdf", "archivo_base64": base64.b64encode(archivo.read()).decode('utf-8')}
