...
# URL PROPORCIONADA POR SERGIO (VERSIÓN ACTUALIZADA)
...
# MAPEOS DE ESTADOS Y COLORES CORPORATIVOS
...
# --- FUNCIÓN: VALIDACIÓN MATEMÁTICA DE RUT CHILENO ---
def validar_rut(rut):
...
with tabs[0]:
    ...
    if not df_av.empty:
        ...
        # Matemática de Cumplimiento (Excluyendo Estado 9)
        ...
        with c_iz:
            ...
            draw_grid(m1); st.write(""); draw_grid(m2)

        with c_de:
            st.subheader("📄 Certificado")
            m_pdf = st.selectbox("Mes PDF:", cols_m, key="s_pdf_d")
            if st.button("Obtener PDF", use_container_width=True):
                ...
                if not match_id.empty:
                    ...
                    n_f = f"Certificado.{m_pdf}{anio_global}.pdf" # Ajustado para nombre directo
...
with tabs[2]:
    ...
    if mes_sidebar == "AÑO COMPLETO": st.warning("Seleccione un Mes en el sidebar.")
        ...
        if not st.session_state.c_send:
            ...
            st.warning("⚠️ ¿Confirmar envío mensual de documentación?")
            c1, c2 = st.columns(2)
            if c1.button("✅ SÍ, ENVIAR", use_container_width=True):
                ...
                st.session_state.c_send = False; st.success("Notificación enviada.")
            if c2.button("❌ NO, VOLVER", use_container_width=True): st.session_state.c_send = False; st.rerun()

# --- PESTAÑA 4: INGRESO COLABORADOR (RUT VALIDADOR + RECORDATORIO + BULK UPLOAD) ---
with tabs[3]:
    st.header("👤 Registro de Nuevo Colaborador")
    st.info("Complete los datos del trabajador y arrastre todos los archivos requeridos.")
    ...
    with col_inp:
        ...
        with ci1: n_new = st.text_input("Nombre Completo:", placeholder="JUAN PEREZ SOTO")
        with ci2: 
            r_new = st.text_input("RUT (ej: 12345678-9):", placeholder="12345678-9")
            # Validación de RUT integrada
            r_ok = False
            if r_new:
                if validar_rut(r_new):
                    st.caption("✅ RUT Válido")
                    r_ok = True
                else: st.caption("❌ RUT Inválido")
        
        st.divider()
        f_bulk = st.file_uploader("Arrastre aquí todos los documentos del trabajador (PDF):", type=["pdf"], accept_multiple_files=True, key="bulk_up_new")
        
        if f_bulk and n_new and r_ok:
            st.write(f"Archivos listos: {len(f_bulk)}")
            if st.button("Subir Documentación", use_container_width=True):
                ...
                if not mt.empty:
                    ...
                    st.success(f"Se han cargado {len(f_bulk)} documentos correctamente.")
    ...
    if not st.session_state.c_new:
        if st.button("🏁 FINALIZAR INGRESO Y NOTIFICAR", use_container_width=True):
            ...
            else: st.warning("Complete Nombre y RUT correctamente.")
    else:
        ...
        if k1.button("✅ SÍ, NOTIFICAR", use_container_width=True):
            ...
            st.session_state.c_new = False; st.success("Ingreso notificado.")
        if k2.button("❌ VOLVER", use_container_width=True): st.session_state.c_new = False; st.rerun()

# --- PESTAÑA 5: ADMIN (SOLO ROLES NO USUARIO) ---
if rol != "USUARIO":
    with tabs[4]:
