            ...
            if st.button("Ingresar"):
                ...
                if not df_u.empty:
                    ...
                    if not user_match.empty:
                        ...
                        # Log de acceso "invisible" para el usuario
...
with tabs[0]:
    ...
    if not df_av.empty:
        ...
        if df_display.empty:
            st.warning(f"No hay registros para {st.session_state['user_empresa']}.")
        else:
            try:
                ...
                # Matemática de Cumplimiento Real (Excluyendo 9)
...
# --- PESTAÑA 2: KPIs EMPRESAS (RESTRINGIDA) ---
if rol != "USUARIO":
    with tabs[1]:
        st.header("🏢 KPIs Nivel Empresa")
        st.info("Pestaña reservada para indicadores avanzados de empresas.")
    ...
    if anio_global == "2025":
        ...
        if not df_s.empty:
            ...
            if not plazo_f.empty:
                ...
                st.dataframe(plazo_f[['Razón Social', 'Rut Trabajador', 'Nombres', 'Apellido Paterno', 'Tipo Contrato']], use_container_width=True)
