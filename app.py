...
# ==============================================================================
# 1. CONFIGURACIÓN DE PÁGINA Y ESTILOS CSS (OCULTAR 200MB)
# ==============================================================================
st.set_page_config(
...
# Inyección de CSS para ocultar el límite nativo de 200MB y limpiar la UI
st.markdown("""
    <style>
    ...
    /* Mejorar estética de las cajas de información */
...
# IDS DE LAS HOJAS DE CÁLCULO (DATABASE)
ID_AVANCE = "1H-L5zzWlm1_bubJab3G_kztzWBfgUZuPnFvrbcFvj7Y"
ID_EMPRESAS = "1sC0BNZTc1UuOVhl9UqaBqCehuXso3AxqBVwQ7tm4Ybo" 
ID_USUARIOS = "1FnjiFO_m2h1BqlzNFnR5AQhBY8924MrAg-QP8oZV7CY"
ID_COLABORADORES = "1EAJF1P2W2cFkl-QvD6RwTpms-_R_aYeabDZxIyOB4W0"

# MAPEOS DE DATOS CORPORATIVOS
MAPA_ESTADOS = {
...
# ==============================================================================
# 2. FUNCIONES DE APOYO (RUT Y CARGA)
# ==============================================================================
def validar_rut(rut):
    """Verifica si el RUT chileno ingresado es matemáticamente correcto."""
...
if "authenticated" not in st.session_state:
    ...
    with c2:
        ...
        if st.button("Ingresar al Portal", use_container_width=True):
            ...
            if not df_u.empty:
                # SE CORRIGE LA COLUMNA DE BÚSQUEDA PARA EVITAR EL ERROR col_clave
                col_clave_check = next((c for c in df_u.columns if 'CLAVE' in str(c).upper()), 'CLAVE')
                match = df_u[df_u[col_clave_check].astype(str).str.strip() == pwd]
                if not match.empty:
                    u = match.iloc[0]
...
# ==============================================================================
# 5. DEFINICIÓN DE PESTAÑAS SEGÚN ROL (EECC SOLO VE 1, 3 Y 4)
# ==============================================================================
rol = st.session_state["u_rol"]

if rol == "USUARIO":
    # EECC solo ven Dashboard, Carga Mensual y Dotación
...
# ==============================================================================
# 6. CONTENIDO DE LAS PESTAÑAS (EXPANDIDO)
# ==============================================================================
...
with tabs[tab_list.index("📉 Dashboard")]:
    ...
    if not df_av.empty:
        ...
        # Filtro de datos: Usuarios EECC solo ven su propia fila
        ...
        with c_de:
            ...
            if st.button("Obtener PDF", use_container_width=True):
                ...
                if not match_id.empty:
                    ...
                    else: st.error("Certificado no encontrado en Drive.")
...
with tabs[tab_list.index("📤 Carga Mensual")]:
    ...
    if mes_sidebar == "AÑO COMPLETO":
        st.warning("Seleccione un Mes en el sidebar para habilitar el cargador.")
    else:
        ...
        with col_m_inp:
            ...
            # Lista explícita de documentos para asegurar la extensión del código
            m_docs = [
                ("Liquidaciones Sueldo", "LIQ", ["pdf"]),
                ("Comprobantes Pago", "PAGOS", ["pdf"]),
                ("Cotizaciones", "PREVIRED", ["pdf"]),
            ...
            for n, p, e in m_docs:
                ...
                with cb:
                    ...
                    if st.button("Subir", key=f"bm_{p}", use_container_width=True):
                        if a:
                            if a.size > 20*1024*1024: st.error("Archivo excede los 20MB.")
                            else:
...
with tabs[tab_list.index("👥 DOTACION")]:
    ...
    with col_d_inst:
        ...
        txt_d = "<li>Contrato / Anexo</li><li>Cédula Identidad</li><li>AFP/Salud</li>" if "Alta" in acc else "<li>Finiquito Firmado</li><li>Anexo Traslado</li>"
        st.markdown(f"""
        ...
        <p style='font-size:14px; color:#d9534f;'><b>⚠️ MÁXIMO 20MB POR ARCHIVO.</b></p>
        <ul style='font-size:14px; line-height:1.6;'>{txt_d}</ul></div>""", unsafe_allow_html=True)
    with col_d_inp:
        ...
        with ci2: 
            r_rut = st.text_input("RUT (con guion):", placeholder="12.345.678-9")
        ...
        if "Alta" in acc:
            ...
            if f_in and n_nom and r_ok:
                if st.button("Subir Alta", use_container_width=True):
                    if any(f.size > 20*1024*1024 for f in f_in): st.error("Archivo(s) exceden 20MB.")
                    else:
            ...
            if f_out and n_nom and r_ok:
                if st.button("Subir Baja", use_container_width=True):
                    if f_out.size > 20*1024*1024: st.error("Supera los 20MB.")
                    else:
                        id_f = str(df_id_empresas[df_id_empresas['EMPRESA'].str.contains(emp_c[:10], case=False, na=False)].iloc[0]['IDCARPETA']).strip()
...
# --- TAB: ADMIN (SOLO ROLES NO USUARIO) ---
if rol != "USUARIO":
