...
# --- CONEXIÓN DRIVE (Tu URL de Apps Script) ---
URL_APPS_SCRIPT = "https://script.google.com/macros/s/AKfycbwDySSNxy2ZKeUtm_UTYKeGechxxNc1Bkmnb43BJ_CfVa7GFpDyKAwndDJN1wPkg9BKrg/exec"
...
if rol == "ADMIN":
    ...
    tabs = st.tabs(["📤 Subir Documentos", "📈 Mi Avance", "👥 Masa Laboral"])

# --- TAB 1: AVANCE + PASARELA ---
with tabs[0]:
    ...
    if not df_av.empty:
        ...
        with st.expander("📤 PASARELA DE CARGA DE DOCUMENTOS"):
            if mes_sidebar == "AÑO COMPLETO":
                ...
                empresa_up = st.session_state['u_emp'] if rol == "USUARIO" else st.selectbox("Empresa:", sorted(df_f[col_e].unique()))
                docs_up = [("Liquidaciones", "LIQ"), ("Planilla Previred", "PREVIRED"), ("F30", "F30"), ("F30-1", "F30_1"), ("Pagos", "PAGOS"), ("Otros", "OTROS")]
                
                for nombre_doc, prefijo in docs_up:
                    col_file, col_btn = st.columns([3, 1])
                    archivo = col_file.file_uploader(f"Subir {nombre_doc}", type=["pdf"], key=f"up_{prefijo}")
                    if col_btn.button(f"Enviar {prefijo}", key=f"btn_{prefijo}"):
                        if archivo:
                            ...
                            if not match.empty:
                                ...
                                id_root = str(match.iloc[0][col_f]).strip()
                                ...
                                payload = {"nombre_final": nombre_f, "id_carpeta": id_root, "anio": anio_global, "mes_nombre": MAPA_MESES_CARPETAS[mes_sidebar], "mimetype": "application/pdf", "archivo_base64": b64}
                                with st.spinner(f"Subiendo..."):
                                    r = requests.post(URL_APPS_SCRIPT, params=payload)
                                    if "✅" in r.text: st.success("OK"); st.balloons()
        ...
        al_dia = ((df_num == 5).all(axis=1)).sum()
...
with tabs[idx_masa]:
    ...
    if not df_masa.empty:
        ...
        if col_con:
            ...
            if not pf.empty: st.warning(f"🚨 Se detectaron {len(pf)} contratos a Plazo Fijo."); st.dataframe(pf, use_container_width=True)
