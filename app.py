...
# ==============================================================================
# 1. CONFIGURACIÓN DE PÁGINA Y LIMPIEZA VISUAL (OCULTAR 200MB)
# ==============================================================================
st.set_page_config(
...
# CSS AVANZADO: Borra el texto de "200MB" y ajusta los cargadores
st.markdown("""
    <style>
    ...
    /* Ajuste de botones para que no choquen */
...
MAPA_MESES_CARP = {
    'ENE':'01_ENE','FEB':'02_FEB','MAR':'03_MAR','ABR':'04_ABR','MAY':'05_MAY','JUN':'06_JUN',
    'JUL':'07_JUL','AGO':'08_AGO','SEP':'09_SEP','OCT':'10_OCT','NOV':'11_NOV','DIC':'12_DIC'}

# ==============================================================================
# 2. FUNCIONES DE APOYO
# ==============================================================================
def validar_rut(rut):
    """Verifica el RUT con algoritmo Módulo 11."""
...
# ==============================================================================
# 3. LOGIN (SOLUCIÓN NameError col_clave)
# ==============================================================================
if "authenticated" not in st.session_state:
    ...
    with c2:
        ...
        if st.button("Ingresar", use_container_width=True):
            ...
            if not df_u.empty:
                # SE DEFINE LA VARIABLE CORRECTAMENTE
...
# ==============================================================================
# 6. DESARROLLO DE PESTAÑAS (EXPANDIDO)
# ==============================================================================

# --- TAB 1: DASHBOARD ---
with tabs[tab_list.index("📉 Dashboard")]:
...
with tabs[tab_list.index("📤 Carga Mensual")]:
    ...
    if mes_sidebar == "AÑO COMPLETO":
        st.warning("Seleccione un Mes en el sidebar para cargar.")
    else:
...
with tabs[tab_list.index("📤 Carga Mensual")]:
    ...
    if mes_sidebar == "AÑO COMPLETO":
        ...
        with col_m_inst:
            ...
            <p style='font-size:14px; color:#d9534f;'><b>⚠️ MÁXIMO 20MB POR ARCHIVO.</b></p>
            <ul style='font-size:14px; line-height:1.6;'>
                <li>Liquidaciones de Sueldos PDF único con todos los colaboradores</li>
                <li>Comprobantes Pago/Anticipos PDF único con todos los colaboradores</li>
                <li>Planillas de Cotizaciones</li>
                <li>Libro Remuneraciones enviado DT(CSV)</li>
                <li>Certificado F30 (Maximo 30 días)</li>
        ...
        with col_m_inp:
            ...
            m_docs = [
                ("Liquidaciones Sueldo", "LIQ", ["pdf"]), ("Comprobantes Pago", "PAGOS", ["pdf"]),
                ("Cotizaciones", "PREVIRED", ["pdf"]), ("Libro Remuneraciones (CSV)", "LIBRO", ["csv"]),
                ("Comprobante Envío DT", "DT_COMP", ["pdf"]), ("Certificado F30", "F30", ["pdf"]),
                ("Certificado F30-1", "F30_1", ["pdf"]), ("Planilla Control (.XLS)", "CONTROL", ["xlsx", "xls"])
            ]
            for n, p, e in m_docs:
                ...
                with cb:
                    ...
                    if st.button("Subir", key=f"bm_{p}", use_container_width=True):
                        if a:
                            if a.size > 20*1024*1024: st.error("⚠️ Excede los 20MB.")
                            else:
                                ...
                                payload = {
                                    ...
                                    "mimetype":"application/pdf" if ext=="pdf" else "text/csv" if ext=="csv" else "application/vnd.ms-excel",
                                    "archivo_base64":base64.b64encode(a.read()).decode('utf-8')
                                }
                                r = requests.post(URL_APPS_SCRIPT, data=payload)
                                if r.status_code == 200: st.success("Cargado.")
            st.divider()
            # SE AGREGA UN KEY ÚNICO PARA EVITAR StreamlitDuplicateElementId
            if st.button("🏁 FINALIZAR Y NOTIFICAR", key="btn_notif_mensual", use_container_width=True):
...
with tabs[tab_list.index("👥 DOTACION")]:
    ...
    with col_d_inst:
        ...
        st.markdown(f"""<div style='background-color:#fefefe; padding:18px; border-radius:12px; border: 1px solid #ddd; border-left: 8px solid {color_b};'><h4 style='margin-top:0;'>📌 Requisitos</h4><p style='font-size:14px; color:#d9534f;'><b>⚠️ MÁXIMO 20MB POR ARCHIVO.</b></p><ul style='font-size:14px;'>{txt_d}</ul></div>""", unsafe_allow_html=True)
    with col_d_inp:
        ...
        with ci2: 
            r_rut = st.text_input("RUT (ej: 12345678-9):", placeholder="12.345.678-9")
            r_ok = validar_rut(r_rut) if r_rut else False
            if r_rut:
                if r_ok: st.caption("✅ RUT Válido")
            ...
            if f_out and n_nom and r_ok:
                if st.button("Subir Baja", use_container_width=True):
                    if f_out.size > 20*1024*1024: st.error("⚠️ Supera los 20MB.")
                        ...
                        st.success("Egreso enviado.")
    st.divider()
    # SE AGREGA UN KEY ÚNICO PARA EVITAR StreamlitDuplicateElementId
    if st.button("🏁 FINALIZAR Y NOTIFICAR", key="btn_notif_dotacion", use_container_width=True):
        if n_nom and r_ok:
